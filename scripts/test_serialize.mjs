...
try {
  ...
  console.log('unprovenCallTxData keys:', Object.keys(unprovenCallTxData));
  console.log('unprovenCallTxData.public keys:', Object.keys(unprovenCallTxData.public));
  console.log('unprovenCallTxData.private keys:', Object.keys(unprovenCallTxData.private));
  ...
  console.log('Serialized hex length:', hex.length);
  const { Transaction } = await import('@midnight-ntwrk/midnight-js-protocol/ledger');
  const roundTrip = Transaction.deserialize('signature', 'proof', 'pre-binding', Buffer.from(hex, 'hex'));
  console.log('✅ Roundtrip Transaction.deserialize SUCCESSFUL!! Identifiers:', roundTrip.identifiers());
